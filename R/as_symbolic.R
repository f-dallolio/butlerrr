...
#' symbols orcalls (e.g. `"func"` to `func`, `"pkg::func(x, ...)"` to
#' `pkg::func(x, ...)`, etc.`).
...
#'
#' @param x an R object. Precisely, a syntactic literal, a symbol, a call, a
#'   function, or a formula.
#' @param ... R objects as above.
#' @param .x a list of R objects. It gets appended to `...`.
#' @param .named TRUE or FALSE. If `TRUE` (default), the unnamed elements of the
#'   output of `as_symbolic_objs` are automatically named.
#' @param .name_all TRUE or FALSE. If `TRUE` (default), all elements of the
#'   output of `as_symbolic_objs` are automatically named.
#'
#'
